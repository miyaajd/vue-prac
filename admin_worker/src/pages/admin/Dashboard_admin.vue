  ...
  <div class="space-y-6 bg-white text-black dark:bg-black dark:text-white p-4 rounded">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
        ...
        <div class="space-y-4">
          <div
            v-for="(reser, i) in recentReservations"
            :key="reser.id"
            class="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
            <div>
              <p class="font-medium text-gray-900 dark:text-white">{{ reser.customerName }}</p>
              <p class="text-sm text-gray-500 dark:text-gray-400">{{ reser.date }}</p>
            </div>
            <span :class="getStatusClass(reser.status)" class="px-2 py-1 text-xs font-semibold rounded-full">
              {{ reser.status }}
            </span>
          </div>
        </div>
