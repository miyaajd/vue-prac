<template>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
      ...
      <div class="bg-white rounded-xl shadow-sm text-gray-700 dark:text-gray-300">
        ...
        <div class="p-6 space-y-6">
          <div
            v-for="notifi in notificationSettings"
            :key="notifi.key"
            class="flex justify-between items-center">
            <div class="flex-1">
              <h3 class="text-gray-800 font-medium mb-1">{{ notifi.title }}</h3>
              <p class="text-sm text-gray-500">{{ notifi.description }}</p>
            </div>
            <!-- ********************토글버튼 (테일윈드)*************************************** -->
            <label class="relative inline-flex items-center cursor-pointer ml-4">
              <input type="checkbox" class="sr-only peer" v-model="settings[notifi.key]" />
              <div
  ...
  <div
    v-if="showToast"
    class="fixed bottom-4 right-11 px-6 py-3 bg-green-500 text-white rounded-lg shadow-lg animate-slide-in flex items-center gap-2 z-50">
